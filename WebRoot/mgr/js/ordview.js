$(function() {
	...
	function loadcontent(rel, obj) {
		...
		for (var i = 0; i < data.length; i++) {
			str = "<div class='order-detail'><div class='col-md-5'><label>订单编号：</label><h5>" + data[i].ORD_ID + "</h5></div>" +
					...
					"<h5>" + data[i].ORD_TEL + "</h5></div><div class='col-md-4'><label>　收货人：</label><h5>" + data[i].ORD_NAME + "</h5></div>" +
					"<div class='col-md-3'><label>订单状态：</label><h5>" + data[i].ORD_STATUSNAME + "</h5></div><div class='col-md-12'>" +
					"<label>图书名称：</label><h5>" + data[i].ORD_TITLE + "</h5></div><div class='col-md-10'><label>收货地址：</label>" +
					"<h5>" + data[i].ORD_ADDRESS + "</h5></div><div class='col-md-2 order-btngroup'><div>" +
	...
	function load() {
		$.ajax({
			url: "../Order.do",
	        data: "act=loadorderformgr&issell=" + issell + "&status=" + status + "&ordid=" + ordid + "&starttime=" + starttime + "&endtime=" + endtime + "&page=" + currpage,
	        type: "POST",
	...
	function loadPageControl() {
		$.ajax({
	        url: "../Order.do",
	        data: "act=mgrordtotalpage&issell=" + issell + "&status=" + status + "&ordid=" + ordid + "&starttime=" + starttime + "&endtime=" + endtime,
	        type: "POST",
	        success: function (rel) {
	        	//加载页码
