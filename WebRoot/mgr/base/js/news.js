$(function () {		
	...
	function loadcontent(rel) {
		...
		for (var i = 0; i < data.length; i++) {				
			str = "<tr><th scope='row'>" + data[i].NTC_ID + "</th><td>" + data[i].NTC_TYPE + "</td><td>" + data[i].NTC_TITLE + 
					"</td><td>" + data[i].NTC_AUTHOR + "</td><td>" + data[i].NTC_ADDTIME + "</td><td class='col-control'>" +
					"<button class='btn btn-success' data-toggle='modal' data-target='#edit-dialog' data-id='" + data[i].NTC_ID + 
					"' onclick='editload(this);'>编辑</button><button data-id='" + data[i].NTC_ID + "' data-title='" + 
					data[i].NTC_TITLE + "' data-toggle='modal' data-target='#del-dialog' class='btn btn-danger' " +
	...
	function load() {
		$.ajax({
			url: "../../Article.do",
			data: "act=load&tab=news&title=" + title + "&type=" + type + "&starttime=" + starttime + "&endtime=" + endtime + "&page=" + currpage,
			type: "post",
	...
	function loadPageControl() {
		$.ajax({
	        url: "../../Article.do",
	        data: "act=noticetotalpage&title=" + title + "&type=" + type + "&starttime=" + starttime + "&endtime=" + endtime,
	        type: "POST",
	        success: function (rel) {
	        	//加载页码
