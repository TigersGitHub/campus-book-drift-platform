	...
	<head>
        ...
        <script type="text/javascript">
        	function delload(obj) {
        		...
        		$("#del-article").attr("href", "../../Article.do?act=del&tab=news&ntcid=" + id);
        	}
	    	...
	    	function editload(obj) {
	    		...
	    		$.ajax({
	    			url: "../../Article.do",
	    			data: "act=editload&tab=news&ntcid=" + id,
	    			type: "post",
	    			success: function (rel) {
	    				var data = eval(rel);
	    				$("#article-id").val(id);
	    				$("#article-type-edit").val(data[0].NTC_TYPE);
	    				$("#article-title-edit").val(data[0].NTC_TITLE);
	    				$("#article-content-edit").text(data[0].NTC_CONTENT);
	    				$("#author-edit").val(data[0].NTC_AUTHOR);
	    				$("#datetimebox-edit").val(data[0].NTC_ADDTIME);
	    				$("#article-note-edit").val(data[0].NTC_NOTE);
	    			}
	    		});
	    	}
        </script>
    ...
    <body id="news-mgr">
			...
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					<div class="modal-header">
						...
						<h4 class="modal-title">写新闻公告</h4>
					</div>
					<div class="modal-body">
						<form id="form-addarticle" action="<%=path %>/Article.do?act=add&tab=news" method="post">						
							<div class="input-group">
						    	<select id="article-type" name="article-type" class="input-group-addon form-control">
    	...
    	<div class="modal fade" id="edit-dialog" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					...
					<div class="modal-body">
						<form id="form-editarticle" action="<%=path %>/Article.do?act=edit&tab=news" method="post">
							<input type="hidden" id="article-id" name="article-id" />
							<div class="input-group">
						    	<select id="article-type-edit" name="article-type-edit" class="input-group-addon form-control">
						    		<option value="N">新闻</option>
						    		<option value="A">公告</option>
						    	</select>
			...
			<div class="modal-dialog modal-sm" role="document">
				<div class="modal-content">
					<div class="modal-header">
						...
						<h4 class="modal-title">删除新闻公告</h4>
					</div>
					<div class="modal-body">
		...
		<div class="card list-tool">
			<div class="card-block">
				<button class="btn btn-success" data-toggle="modal" data-target="#add-dialog">+&nbsp;写新闻公告</button>
